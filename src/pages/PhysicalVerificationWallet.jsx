...
export const PhysicalVerificationWallet = () => {
  const [landData, setLandData] = useState([]);
  const [selectedRow, setSelectedRow] = useState(null);
  const [landDetails, setLandDetails] = useState(null);
  const [loading, setLoading] = useState(true);
  const [expandedRows, setExpandedRows] = useState(new Set());
  const [activeTab, setActiveTab] = useState("physicalVerificationWallet");
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);

  useEffect(() => {
    if (activeTab === "physicalVerificationWallet") {
      fetchLandWallet();
    }
  }, [activeTab]);
  ...
  const fetchLandWallet = async () => {
    try {
      setLoading(true);
      const res = await axios.get("http://72.61.169.226/admin/physical/wallet", {
        headers: { Authorization: `Bearer ${token}` },
      });
      setLandData(res.data.data);
    } catch (error) {
      console.error("Error fetching Physical Verification wallet data:", error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-emerald-50">
      ...
      <header className="hidden lg:flex h-14 items-center justify-between bg-white px-6 shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-semibold">Physcial Verification Wallet</h1>
        </div>
      </header>
      <div className="p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8">
            <div className="flex space-x-4">
              <button
                onClick={() => setActiveTab("physicalVerificationWallet")}
                className={`
                  px-6 py-3 rounded-lg font-medium text-sm transition-all duration-200
                  ${activeTab === "physicalVerificationWallet" 
            ...
            <div className="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-emerald-50">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold text-gray-800">
                  Physical Verification Wallet Records:
                </h2>
