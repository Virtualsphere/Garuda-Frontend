...
import { 
  ...
  FiAlertCircle, FiLoader, FiClock, FiBriefcase,
  FiChevronDown, FiChevronUp, FiUser, FiCalendar 
} from "react-icons/fi";
import { toast, Toaster } from "react-hot-toast";

export const UpdateJobModal = ({ row, onClose, onSuccess }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const updateData = { 
        status,
        job_post_amount: parseFloat(amount)
      };

      const endpoint = `http://72.61.169.226/admin/job/wallet/${row.id || row.job_post_wallet_id}`;
      ...
      toast.success("Job Post Wallet updated successfully!");
      onSuccess();
      setTimeout(() => {
        onClose();
      }, 1000);
    ...
    return (
      <div
        ...
        className={`
          ...
          border-2 ${status === option.value ? 'border-indigo-300 bg-indigo-50' : 'border-gray-100 hover:border-gray-200'}
          hover:shadow-sm
        `}
      >
    ...
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-70 z-50 p-4 backdrop-blur-sm overflow-y-auto">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col animate-fadeInUp my-4">
          {/* Header */}
          <div className="px-6 py-4 border-b border-gray-100 flex-shrink-0 bg-gradient-to-r from-indigo-50 to-white">
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-800">Update Job Post Wallet</h3>
                  <p className="text-sm text-gray-600 mt-1">Job Post Amount for {row.name}</p>
                </div>
              </div>
              <button
                onClick={onClose}
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 transition-all"
                    placeholder="Enter job post amount"
                    min="0"
                    step="1"
                  />
                ...
                <p className="text-xs text-gray-500">
                  This amount will be sent as job_post_amount field
                </p>
              </div>
          ...
          <div className="px-6 py-4 border-t border-gray-100 flex-shrink-0 bg-gray-50 rounded-b-2xl">
            <div className="flex flex-col space-y-3">
              <div className="text-xs text-gray-500 text-center">
                Updating: <span className="font-mono font-medium">job_post_amount</span> field
              </div>
                ...
                <button
                  ...
                  className={`
                    ...
                    ${loading 
                      ...
                      : 'bg-gradient-to-r from-indigo-500 to-violet-600 hover:from-indigo-600 hover:to-violet-700 hover:shadow-lg'
                    }
                  `}
                >
                  {loading ? (
