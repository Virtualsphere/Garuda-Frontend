...
import { 
  FiX, FiSend, FiDollarSign, FiCheckCircle, 
  FiAlertCircle, FiLoader, FiClock, FiTrendingUp,
  FiChevronDown, FiChevronUp, FiUser, FiCalendar 
} from "react-icons/fi";
import { toast, Toaster } from "react-hot-toast";

export const UpdateAdsModal = ({ row, onClose, onSuccess }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const updateData = { 
        status,
        ads_amount: parseFloat(amount)
      };

      const endpoint = `http://72.61.169.226/admin/ads/wallet/${row.id || row.ads_wallet_id}`;
      ...
      toast.success("Ads Wallet updated successfully!");
      onSuccess();
      setTimeout(() => {
        onClose();
      }, 1000);
    ...
    return (
      <div
        ...
        className={`
          ...
          border-2 ${status === option.value ? 'border-cyan-300 bg-cyan-50' : 'border-gray-100 hover:border-gray-200'}
          hover:shadow-sm
        `}
      >
    ...
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-70 z-50 p-4 backdrop-blur-sm overflow-y-auto">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col animate-fadeInUp my-4">
          {/* Header */}
          <div className="px-6 py-4 border-b border-gray-100 flex-shrink-0 bg-gradient-to-r from-cyan-50 to-white">
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <div className="p-2 bg-cyan-100 rounded-lg mr-3">
                  <FiTrendingUp className="text-cyan-600 text-lg" />
                </div>
                <div>
                  <h3 className="text-xl font-bold text-gray-800">Update Ads Wallet</h3>
                  <p className="text-sm text-gray-600 mt-1">Ads Amount for {row.name}</p>
                </div>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-2">
                <label className="block text-sm font-semibold text-gray-700 flex items-center">
                  <FiDollarSign className="mr-2 text-cyan-500" />
                  Ads Amount (₹)
                  <span className="ml-2 text-xs px-2 py-1 bg-cyan-100 text-cyan-700 rounded-full">
                    Per Ad Campaign
                  </span>
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-cyan-300 focus:border-cyan-400 transition-all"
                    placeholder="Enter ads amount"
                    min="0"
                    step="1"
                  />
                ...
                <p className="text-xs text-gray-500">
                  This amount will be sent as ads_amount field
                </p>
              </div>
          ...
          <div className="px-6 py-4 border-t border-gray-100 flex-shrink-0 bg-gray-50 rounded-b-2xl">
            <div className="flex flex-col space-y-3">
              <div className="text-xs text-gray-500 text-center">
                Updating: <span className="font-mono font-medium">ads_amount</span> field
              </div>
                ...
                <button
                  ...
                  className={`
                    ...
                    ${loading 
                      ...
                      : 'bg-gradient-to-r from-cyan-500 to-teal-600 hover:from-cyan-600 hover:to-teal-700 hover:shadow-lg'
                    }
                  `}
                >
                  {loading ? (
